...
import akka.testkit.javasdl.TestKit;
import jay.mars.market.messages.CreateSequence;
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    within(Duration.ofSeconds(5), () -> {
                        for (int i = 0; i < 10; i++) {
                            server.tell(message, getRef());
                            awaitCond(this::msgAvailable);
                            expectMsgPF("expect get next and next value", msg -> {
                                Assert.assertEquals(lastId.get() + 1, msg);
                                lastId.set((Long)msg);
                                return msg;
                            });
